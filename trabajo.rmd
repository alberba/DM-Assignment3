...
Como hemos podido ver, el modelo ha mejorado, pero creemos que se puede optimiza el modelo. Por tanto, vamos a ver que variables son las más importantes poder optimizar el modelo sin perjudicar la precisión.
...
The results are similar to the ID3 model, athey are a bit better but with a small margin. These are good results, but we should keep checking every tree model to see which one is the best.
