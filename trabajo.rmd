...
Now we can see that the variable 'person_age' has no correlation with other variables or with the target 
variable. Therefore, it does not contribute to the model, so we will proceed to remove it.
...
accurately predict whether a loan would be approved or not.

Our first task was dealing with the data, due to the fact that our dataset contained 45000 loan applications 
with 13 different attributes numerical and categorical and an objective variable telling us if the loan was 
approved or rejected.

As we explored the data, we discovered some 'odds' like applicants with unrealistically high ages and 
...
Using differents plots and transformations we examined how each variable behaved and how it related to the 
...
As has been said before, we applied transformations like logarithms and square roots to improve the skewness, 
...
With our data prepped and ready, we built and evaluated several classification models.
...
We explored various algorithms, including ID3, C5.0, CART, and we used packages like rpart and tree.
...
sometimes even reduced accuracy due to added complexity.
...
After extensive testing with different seeds and train-test splits, it became evident that the Decision Tree using the C5.0 algorithm
consistently outperformed the other models. 

Across various seeds and data splits, the C5.0 model maintained an accuracy of around 90.2% to 90.7% and an F1-Score 
in the vicinity of 0.938 to 0.941.

### Our Learnings

We realized how crucial it is to start with clean, accurate data. Handling outliers and missing values can dramatically affect model performance.

Our efforts in normalizing distributions and removing multicollinear variables paid off.

By experimenting with different algorithms, we saw firsthand how models like Naive Bayes, KNN, and decision trees behave under various conditions. 
