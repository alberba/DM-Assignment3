...
Vemos que hay 6 muestras con un porcentaje de préstamo sobre los ingresos anuales de 0%. Esto es imposible, ya que si el porcentaje es 0%, el préstamo es 0$. Por tanto, eliminaremos estas observaciones.
...
numeric_Corr <- data[, c('person_age', 'person_income', 'person_emp_exp', 'loan_amnt', 'loan_int_rate', 'loan_percent_income', 
                         'cb_person_cred_hist_length', 'credit_score')]

ggcorr(numeric_Corr, label = TRUE)
```
Como podemos observar, 'credit_score' y 'loan_int_rate', apenas influyen, ya que sus correlaciones con las otras variables son prácticamente nulas.
Por tanto procedemos a eliminarlas.
...
data_Mod <- data[, -c(9, 12)]

# Verificar las columnas restantes
names(data_Mod)
```
```{r}
numeric_Corr <- data[, c('person_age', 'person_income', 'person_emp_exp', 'loan_amnt', 'loan_percent_income', 
...
p1 <- ggplot(data) + geom_boxplot(aes(x = factor(loan_status), y = person_age), fill = "violet", alpha = 0.6) +
...
```{r}
p2 <- ggplot(data) + geom_boxplot(aes(x = factor(loan_status), y = person_income), fill = "#FFB6C1", alpha = 0.6) +
  theme(axis.title = element_text(size = 8.5)) +
...
```{r}
p3 <- ggplot(data) + geom_boxplot(aes(x = factor(loan_status), y = person_emp_exp), fill = "#90EE90", alpha = 0.6) +
  theme(axis.title = element_text(size = 8.5)) +
  labs(title = "Loan Status vs Employment Experience", x = "Loan Status", y = "Employment Experience") +
  theme_minimal()
...
```{r}
p4 <- ggplot(data) + geom_boxplot(aes(x = factor(loan_status), y = loan_amnt), fill = "violet", alpha = 0.6) +
  theme(axis.title = element_text(size = 8.5)) +
...
```{r}
p5 <- ggplot(data) + geom_boxplot(aes(x = factor(loan_status), y = loan_percent_income), fill = "#FFDAB9", alpha = 0.6) +
  theme(axis.title = element_text(size = 8.5)) +
...
```{r}
p6 <- ggplot(data) + geom_boxplot(aes(x = factor(loan_status), y = cb_person_cred_hist_length), fill = "skyblue", alpha = 0.6) +
  theme(axis.title = element_text(size = 8.5)) +
  labs(title = "Loan Status vs Credit History Length", x = "Loan Status", y = "Credit History Length") +
  theme_minimal()
