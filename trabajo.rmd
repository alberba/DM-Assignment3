...
# Identificar las variables numéricas automáticamente
numeric_vars <- names(data_Mod)[sapply(data_Mod, is.numeric)]

# Crear un gráfico para cada variable numérica
for (var in numeric_vars) {
  p <- ggplot(data_Mod, aes_string(x = var)) +
    geom_histogram(fill = "lightblue", color = "black", bins = 30) + # Histograma para variables numéricas
    labs(title = paste("Distribución de", var), x = var, y = "Frecuencia") +
    theme_minimal() +
...
# Crear el modelo
ID3_1 <- J48(loan_status ~ person_age + person_income + loan_amnt + loan_percent_income, data = data_Mod.train1)
plot(ID3_1)
```
...
Por tanto, vamos a reducir el número de intervalos, para mejorar eso.
...
Para definir esto nuevos intervalos nos hemos centrado en los rangos donde había una mayor concentración de los datos.
...
# Crear el modelo
ID3_2 <- J48(loan_status ~ person_age + person_income + loan_amnt + loan_percent_income, data = data_Mod.train2)
plot(ID3_2)
```
...
ID3_Sel <- J48(loan_status ~ previous_loan_defaults_on_file + loan_percent_income + person_home_ownership + person_income + loan_amnt, 
...
Como vemos ha habido mejora, etc etc, comentar los results, imagino que el grafico se puede mejorar para que se vea mejor con las cosas más pequeñas
o la letra más pequeña o algo del estilo.
